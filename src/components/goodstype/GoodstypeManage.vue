<template>
  <div>
    <div style="margin-bottom: 5px">
      ...
      <el-button type="primary" style="margin-left: 10px" @click="exportGoodsType"
        >导出</el-button
      >
    </div>
    <el-table
    ...
    >
      ...
      <el-table-column prop="name" label="物品分类名称" width="180"> </el-table-column>
...
export default {
  ...
  data() {
    return {
      ...
      uploadUrl: this.$httpUrl + '/goodstype/import',
      form: {
        id: "",
        name: "",
        remark: "",
      ...
      rules: { 
        name: [{ required: true, message: "请输入物品分类名称", trigger: "blur" }],
      },
    };
  },
  methods: {
    ...
    exportGoodsType(){
      this.$axios({
        url: this.$httpUrl + "/goodstype/export",
        method: "get",
        responseType: "blob",
      })
        .then((response) => {
          saveAs(new Blob([response.data]), "物品分类信息.xlsx");
    ...
    del(id) {
      console.log(id);
      this.$axios
        .get(this.$httpUrl + "/goodstype/del?id=" + id, this.form)
        //.then((res) => res.data)
        .then((res) => {
          console.log(res);
          if (res.status == 200) {
